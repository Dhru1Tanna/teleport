...
type HostUsers interface {
	...
	DeleteAllUsers(*SessionRegistry) error
	// UserCleanup starts a periodic user deletion cleanup loop for
	// users that failed to delete
	UserCleanup(*SessionRegistry, types.Semaphores, types.SemaphoreLease)
...
func (u *HostUserManagment) DeleteAllUsers(reg *SessionRegistry) error {
...
// UserCleanup starts a periodic user deletion cleanup loop for
// users that failed to delete
func (u *HostUserManagment) UserCleanup(reg *SessionRegistry, s types.Semaphores, lease types.SemaphoreLease) {
	defer s.CancelSemaphoreLease(u.ctx, lease)
	if err := u.DeleteAllUsers(reg); err != nil {
	...
	for {
		...
		lease.Expires = time.Now().Add(time.Minute * 8)
		if err := s.KeepAliveSemaphoreLease(u.ctx, lease); err != nil {
			log.Errorf("Error keeping semaphore lease: %s", err)
			return
		}
	}
}
